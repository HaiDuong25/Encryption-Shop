                ...
                <tr>
                    ...
                    <td>
                        @if($payment->status === 'pending')
                            <span class="badge bg-warning text-dark" style="background-color: #ffc107;">Chờ xác nhận</span>
                        @elseif($payment->status === 'confirmed')
                            <span class="badge bg-success text-white" style="background-color: #28a745;">Đã xác nhận</span>
                        @elseif($payment->status === 'rejected')
                            <span class="badge bg-danger text-white" style="background-color: #dc3545;">Đã hủy</span>
                        @else
                            ...
                            <div class="d-flex align-items-center" style="gap: 6px;">
                                <form action="{{ route('payments.confirm', $payment->id) }}" method="POST"
                                    ...
                                    <button type="submit" class="btn btn-success btn-xs px-2 py-1"
                                        style="font-size: 0.85rem; background-color: #28a745; border-color: #28a745;">Xác
                                        nhận</button>
                                </form>
                                <form action="{{ route('payments.reject', $payment->id) }}" method="POST"
                                    ...
                                    <button type="submit" class="btn btn-danger btn-xs px-2 py-1"
                                        style="font-size: 0.85rem; background-color: #dc3545; border-color: #dc3545;">Hủy
                                        đơn</button>
                                </form>
                            </div>
                        @elseif($payment->status === 'confirmed')
