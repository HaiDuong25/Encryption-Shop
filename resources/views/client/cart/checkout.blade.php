            ...
            <div class="row align-items-center">
                <div class="col-md-8">
                    <div class="d-flex align-items-center">
                        <i class="fa-solid fa-shopping-cart me-3 text-primary"
                            style="font-size: 28px; color: #ee4d2d !important;"></i>
                        <div>
            ...
            <div class="form-section">
                ...
                <div class="row">
                    ...
                    @foreach ($carts as $cart)
                        @php
                            $price =
                                $cart->variant->sale_price ??
                                ($cart->variant->price ?? ($cart->product->sale_price ?? $cart->product->price));
                            ...
                            <div class="product-item">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        @if ($cart->variant)
                                            @php
                                                $currentSize = $cart->variant->attributeValues
                                                    ->where('attribute.name', 'Size')
                                                    ->first();
                                                $currentColor = $cart->variant->attributeValues
                                                    ->where('attribute.name', 'Màu')
                                                    ->first();
                                                $variantInfo = [];
                                                if ($currentSize) {
                                            ...
                                            @if (!empty($variantInfo))
                                                <small class="text-muted"><i
                                                        class="fa-solid fa-tag me-1"></i>{{ implode(', ', $variantInfo) }}</small>
                                            @endif
                                        @endif
                                    </div>
                                    <div class="col-md-2 text-center">
                                        ...
                                        <div class="price-display">
                                            @if ($salePrice && $salePrice < $originalPrice)
                                                ...
                                                <div class="text-muted small text-decoration-line-through">
                                                    {{ format_vnd($originalPrice) }} VNĐ</div>
                                            @else
                                    ...
                                    <div class="col-md-2 text-center">
                                        <div class="fw-bold fs-5" style="color: #ee4d2d;">{{ format_vnd($itemTotal) }} VNĐ
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row">
                                                    ...
                                                    <div class="card-body">
                                                        ...
                                                        @if ($address->note)
                                                            <p class="text-muted small mt-2 mb-0">
                                                                <i
                                                                    class="fa-solid fa-sticky-note me-1"></i>{{ $address->note }}
                                                            </p>
                                                        @endif
                                                    </div>
                                                </div>
                                    ...
                                    <div class="alert alert-warning">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                ...
                                                <p class="mb-0">Bạn cần thêm ít nhất một địa chỉ giao hàng để tiếp tục đặt
                                                    hàng.</p>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-4 mb-3">
                                <label class="form-label fw-semibold"><i class="fa-solid fa-sticky-note me-1"></i>Ghi chú
                                    đơn hàng</label>
                                <textarea name="notes" class="form-control" rows="3" placeholder="Ghi chú thêm về đơn hàng (tùy chọn)"></textarea>
                            </div>
                            <div class="col-md-4 mb-3">
                                ...
                                <!-- Voucher selection -->
                                <div class="voucher-section-checkout" id="voucher-selection-section"
                                    style="{{ $appliedCoupon ? 'display: none;' : '' }}">
                                    <div class="voucher-header d-flex align-items-center justify-content-between mb-2"
                                        id="voucher-header-checkout">
                                    ...
                                    <div class="input-group mb-2 {{ $appliedCoupon ? 'coupon-applied' : '' }}"
                                        id="coupon-select-group">
                                        <select class="form-select" id="coupon-select-checkout"
                                            {{ $appliedCoupon ? 'disabled' : '' }}>
                                            <option value="">-- Đang tải mã giảm giá... --</option>
                                        </select>
                                        <button
                                            class="btn {{ $appliedCoupon ? 'btn-outline-danger' : 'btn-outline-secondary' }}"
                                            type="button" id="apply-coupon-select"
                                            {{ $appliedCoupon ? '' : 'disabled' }}>
                                            @if ($appliedCoupon)
                                                ...
                                                <div class="row g-1 small">
                                                    ...
                                                    <div class="col-6">
                                                        <strong>Giảm giá:</strong> <span
                                                            id="detail-discount-checkout">-</span>
                                                    </div>
                                                    <div class="col-12">
                                                        <strong>Mô tả:</strong> <span
                                                            id="detail-description-checkout">-</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <!-- Manual input option -->
                                    <div class="mt-2" id="manual-input-toggle"
                                        style="{{ $appliedCoupon ? 'display: none;' : '' }}">
                                        <small class="text-muted">
                                            <a href="#" id="toggle-manual-input" class="text-primary">Hoặc nhập mã
                                                thủ công</a>
                                        </small>
                                    </div>
                                    ...
                                    <div id="manual-input-section" class="mt-2" style="display: none;">
                                        <div class="input-group {{ $appliedCoupon ? 'coupon-applied' : '' }}"
                                            id="coupon-input-group">
                                            <input type="text" class="form-control" id="coupon-input"
                                                name="coupon_code" value="{{ $appliedCoupon ?? '' }}"
                                                placeholder="Nhập mã voucher..." {{ $appliedCoupon ? 'readonly' : '' }}>
                                            <button
                                                class="btn {{ $appliedCoupon ? 'btn-outline-danger' : 'btn-outline-secondary' }}"
                                ...
                                <!-- Status message -->
                                <div class="mt-2" id="coupon-status-message-checkout"
                                    style="{{ $appliedCoupon ? 'display: none;' : '' }}">
                                    <small class="text-muted">
                                        <i class="fa-solid fa-info-circle me-1"></i>
                                                        ...
                                                        <small class="text-muted">
                                                            ...
                                                            @elseif($method->payment_type == 'Số dư ví')
                                                                @php
                                                                    $wallet =
                                                                        auth()->user()->wallet ??
                                                                        auth()->user()->getOrCreateWallet();
                                                                @endphp
                                                                Số dư hiện tại: <span
                                                                    class="fw-bold text-success">{{ number_format($wallet->balance ?? 0, 0, ',', '.') }}
                                                                    VND</span>
                                                                @if (($wallet->balance ?? 0) < $total)
                                                                    <div class="text-danger small mt-1">
                                                                        <i class="fas fa-exclamation-triangle me-1"></i>
                                                                        Số dư không đủ. Cần thêm:
                                                                        {{ number_format($total - ($wallet->balance ?? 0), 0, ',', '.') }}
                                                                        VND
                                                                        <a href="{{ route('wallet.topup') }}"
                                                                            target="_blank" class="text-primary">Nạp
                                                                            thêm</a>
                                                                    </div>
                                                                @endif
                                                            @endif
                            ...
                            <div class="row justify-content-center">
                                <div class="col-md-8">
                                    <h5 class="section-title mb-3"><i
                                            class="fa-solid fa-receipt me-2 text-primary"></i>Tóm tắt đơn hàng</h5>
                                    ...
                                    @if ($appliedCoupon && $couponDiscount > 0)
                                        <div class="summary-row" id="discount-row">
                                            ...
                                            <span class="text-success fw-semibold"
                                                id="discount-amount">-{{ format_vnd($couponDiscount) }} VNĐ</span>
                                        </div>
                                    @else
                        ...
                        <div class="d-flex justify-content-center">
                            @auth
                                ...
                                @else
                                    <button type="button" class="btn btn-warning btn-lg px-5 py-3 fw-bold"
                    ...
                    <div class="d-flex gap-2">
                        @auth
                            @if (Auth::user()->address)
                                <button type="button" class="btn btn-outline-primary btn-sm" id="import-account-address"
                                    title="Import họ tên, số điện thoại và tự động chọn vùng miền từ tài khoản">
                ...
                <form action="{{ route('client.addresses.store') }}" method="POST" id="quick-address-form">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="modal_ward" class="form-label">Xã/Phường/Thị trấn <span
                                        class="text-danger">*</span></label>
                        ...
                        <div class="mb-3">
                            <label for="modal_address_detail" class="form-label">Địa chỉ chi tiết <span
                                    class="text-danger">*</span></label>
                            <textarea class="form-control" id="modal_address_detail" name="address_detail" rows="3"
...
@push('scripts')
    <script>
                        ...
                        .catch(error => {
                            ...
                            if (couponSelect) {
                                ...
                                if (!alreadyExists) {
                                    ...
                                    // Update count badge
                                    const couponCountBadge = document.getElementById(
                                        'available-coupons-count-checkout');
                                    if (couponCountBadge) {
                                        const currentCount = parseInt(couponCountBadge.textContent.match(/\d+/)[
                                            0]) || 0;
        ...
        function showCouponSuccess(message, couponInfo) {
            const discountText = couponInfo.type === 'percentage' ? `${couponInfo.value}%` :
                `${parseInt(couponInfo.value).toLocaleString('vi-VN')}đ`;
            couponResult.innerHTML = `
            <div class="alert alert-success py-2 px-3 mb-0 rounded-3">
                <small>
                    <i class="fa-solid fa-check-circle me-1"></i>
                                    ...
                                    if (data.success) {
                                        ...
                                        const validCoupons = (data.coupons || []).filter(coupon => {
                                            if (coupon.expiry_date) {
                                                ...
                                                0); // Set to start of day for comparison
        ...
        function showCouponDetails(option) {
            ...
            if (discountText !== '-' && discountText !== 'Mã đã lưu') {
                if (!discountText.includes('%') && !discountText.toLowerCase().includes('vnđ') && !discountText
                    .toLowerCase().includes('đ')) {
            ...
            document.getElementById('detail-description-checkout').textContent = description ||
                'Mã giảm giá đã lưu từ danh sách';
                        ...
                        fetch('{{ route('cart.validate-coupon') }}', {
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    showCouponMessage(`Mã giảm giá không còn hợp lệ: ${data.message}`,
                                    'danger');
                                    handleCancelCoupon();
        ...
        function showAppliedCouponUI() {
            ...
            if (appliedDisplay) {
                ...
                const appliedCouponCode = appliedCoupon ? appliedCoupon.coupon_info?.code : (couponInput ? couponInput
                    .value : '');
