                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-body">
                        ...
                        <div class="mb-2"><strong>Phương thức thanh toán:</strong>
                            {{ $order->paymentMethod->payment_type ?? 'Chưa chọn' }}</div>
                        <div class="mb-2"><strong>Phương thức vận chuyển:</strong>
                            {{ $order->shipping_method ?? 'Giao hàng tận nơi' }}</div>
                        <div class="mb-2"><strong>Ngày giao dự kiến:</strong>
                            {{ $order->created_at->addDays(3)->format('d/m/Y') }}</div>
                        <hr>
                        <div class="mb-2"><strong>Người nhận:</strong> {{ $order->recipient_name ?? $order->orderer_name }}
                        </div>
                        ...
                        <div class="mb-2"><strong>Email:</strong>
                            {{ $order->orderer_email ?? ($order->user->email ?? 'N/A') }}</div>
                        @if($isCancelled)
                    ...
                    <div class="card-body">
                        ...
                        @if ($order->coupon || $order->coupon_code)
                                            ...
                                            <div class="mb-2">Số tiền giảm:
                                                <strong>-{{ number_format($order->coupon_discount, 0, ',', '.') }}₫</strong></div>
                        @endif
                                ...
                                <form action="{{ route('client.orders.cancel', $order->id) }}" method="POST" class="w-100">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body px-4 py-3">
                                            <div class="mb-3">
                                                <label for="reason" class="form-label fw-semibold">Lý do hủy đơn hàng <span
